...
// must pass pointer by ref
void print_tree_pre_recur(unique_ptr<Node> &root) {
    if (root == nullptr)
        return;
    cout << root->val << " ";
...
void print_tree_pre(unique_ptr<Node> &root) {
    print_tree_pre_recur(root);
    cout << endl;
}


void print_tree_in_recur(unique_ptr<Node> &root) {
    if (root == nullptr)
        return;
...
void print_tree_in(unique_ptr<Node> &root) {
...
void treeReconst(unique_ptr<Node> &root, int *pre, int *in, int len) {
    // end cond
    if (len == 0)
        return;
